...
suspend fun appFiles(offset: Int, limit: Int, query: String): List<AppFile> {
    ...
    val files = if (text.isEmpty()) fileDao.getPage(limit, offset) else fileDao.getPageText("%${escapeLike(text)}%", limit, offset)
...
suspend fun appFileCount(query: String): Int {
    ...
    return if (text.isEmpty()) dao.count() else dao.countText("%${escapeLike(text)}%")
}
