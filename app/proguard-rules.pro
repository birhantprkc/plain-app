...
# ===== Whisper lyrics extraction (JNI name binding) =====
# Native symbol names in libwhisper_jni.so are derived from these class and
# method names; renaming breaks System.loadLibrary linkage.
-keep class com.ismartcoding.plain.platform.WhisperJni { *; }
-keep class com.ismartcoding.plain.platform.WhisperJni$Segment { *; }
-keepnames interface com.ismartcoding.plain.platform.WhisperJni$ProgressListener
-keepclassmembers class * implements com.ismartcoding.plain.platform.WhisperJni$ProgressListener {
    boolean onProgress(int);
}
